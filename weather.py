...
st.metric("Temperature", f"{fmt(cur['temperature_2m'])}° {unit}", f"Feels like {fmt(cur['apparent_temperature'])}° {unit}")  # FIX 4: key name typos + extra ) removed

col1, col2, col3, col4 = st.columns(4)
col1.metric(":cloudy: Humidity", f"{cur['relative_humidity_2m']}%")
col2.metric(":wind_blowing_face: Wind Speed", f"{cur['wind_speed_10m']} km/h {wind_direction(cur['wind_direction_10m'])}")
col3.metric(":droplet: Precipition", f"{cur['precipitation']} mm")
